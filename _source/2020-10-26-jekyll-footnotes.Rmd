...
base_dir <- here::here()
base_url <- '/'
fig_path <- 'images/posts/jekyll-footnotes/'
...
<div class="notice--danger">
  `{{ notice-text | markdownify }}`{=markdown}
</div>
...
output format you want.`[^1]`{=markdown} 

`[^1]:`{=markdown} Pandoc is incredibly powerful, but it's also incredibly opaque and difficult to learn. You can create incredibly fancy PDF and HTML documents in R Markdown without ever having to know anything about Pandoc.


<figure>
  <img src="{{site.url}}/images/posts/jekyll-footnotes/rmarkdownflow.png" alt="R Markdown flowchart"/>
  <figcaption>Image courtesy of <a href="https://rmarkdown.rstudio.com/lesson-2.html">RStudio</a></figcaption>
</figure>
...
footnotes are longer than 72 characters,`[^2]`{=markdown} then Pandoc will split
...
[file](https://github.com/adnan-armouti/adnan-armouti.github.io/blob/master/_source/2020-10-26-jekyll-footnotes.Rmd)
that
generates the `.md`
[file](https://github.com/adnan-armouti/adnan-armouti.github.io/blob/master/_posts/2020-10-26-jekyll-footnotes.md)
that Jekyll uses to generate the HTML your browser
uses to render this page.

`[^2]:`{=markdown} The default output width defined by the `--columns` argument to Pandoc.
...
present in the `.Rmd` file when generating the `.md` file.`[^3]`{=markdown}
Accordingly, our footnotes will be intact and functional in the final web page.

`[^3]: `{=markdown} You can also use `--wrap=none`, which will put every paragraph in a single gigantic line of text.
...
so I went digging for a new built in dataset.`[^4]`{=markdown} I landed on the
[Loblolly pines dataset](https://stat.ethz.ch/R-manual/R-patched/library/datasets/html/Loblolly.html),
which records the height of `r length(unique(Loblolly$Seed))` different
[loblolly pine trees](https://en.wikipedia.org/wiki/Pinus_taeda).`[^5]`{=markdown}

`[^4]:`{=markdown} If you're willing to install additional packages, Allison Horst's [palmerpenguins](https://github.com/allisonhorst/palmerpenguins) package is fantastic and fills much the same educational niche as `iris`. See [here](https://www.meganstodel.com/posts/no-to-iris/) for even more alternatives.

`[^5]:`{=markdown} Fun fact, loblolly pine seeds were carried aboard Apollo 14 and subsequently planted [throughout the US](https://en.wikipedia.org/wiki/Moon_tree).

```{r ggplot, out.width = '100%'}
library(ggplot2)
